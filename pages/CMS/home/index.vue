  ...
  <div class="navbar-spacing padding-top-30">
    <div class="specification">
      <div class="holder">
        ...
        >
          <h3 style="display: flex;align-items: center;">Home CMS</h3>
          <nuxt-link to="/CMS/home/add" style="text-decoration:none">
            <button class="btn btn-red addTrainer">
              <span class="white-text">Add Home CMS</span>
            </button>
          </nuxt-link>
        </div>

        <div class="row">
          <vue-good-table :columns="columns" :rows="home_cms">
...
<script>
  export default {
    ...
    mounted() {
      this.getHomeCMS();
      this.activatedHomeCMS();
      this.$store.dispatch("activatedHomeCMS").then(res => {
        res.data.map(item => {
    ...
    methods: {
      getHomeCMS: function() {
        this.$store.dispatch("getHomeCMS").then(res => {
          this.home_cms = JSON.parse(JSON.stringify(res.data.reverse()));
        });
      },

      activatedHomeCMS: function() {
        this.$store.dispatch("activatedHomeCMS").then(res => {
          res.data.map(item => {
            this.activatedId = item.id;
            $("#" + this.activatedId).attr("checked", true);
      ...
      getId: function(id) {
        var payload = new FormData();
        payload.append("id", id);
        this.$store.dispatch("activateHomeCmsById", payload).then(res => {});
      }
    }
  };
</script>
